...
class QubicMultibandInstrumentTrapezoidalIntegration:
    ...
    def __init__(self, d):
        ...
        Nf, nus_edge150, filter_nus150, deltas150, Delta, Nbbands = compute_freq(150, int(d['nf_sub']/2)-1, relative_bandwidth=self.FRBW)
        Nf, nus_edge220, filter_nus220, deltas220, Delta, Nbbands = compute_freq(220, int(d['nf_sub']/2)-1, relative_bandwidth=self.FRBW)


        W150 = IntegrationTrapezeOperator(nus_edge150)
        deltas150_trap = np.array([W150.operands[i].todense(shapein=1)[0][0] for i in range(len(nus_edge150))])
        W220 = IntegrationTrapezeOperator(nus_edge220)
        deltas220_trap = np.array([W220.operands[i].todense(shapein=1)[0][0] for i in range(len(nus_edge220))])
        ...
        if not d['center_detector']:
            ...
            for i in range(len(nus_edge150)):
                if self.d['debug']:
                    print(f'Integration done with nu = {nus_edge150[i]} GHz with weight {delta_nu_over_nu_150[i]}')
                #print(nus_edge150)
            ...
            for i in range(len(nus_edge220)):
                if self.d['debug']:
                    print(f'Integration done with nu = {nus_edge220[i]} GHz with weight {delta_nu_over_nu_220[i]}')
                #print(nus_edge220)
