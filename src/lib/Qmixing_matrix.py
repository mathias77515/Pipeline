...
class MixingMatrix(tuple):
    ...
    def params(self):
        ...
        return ["%s.%s" % (type(c).__name__, p) for c in self for p in c.params]
    ...
    def diff(self, nu, *params):
        ...
        for i_c, c in enumerate(self):
            param_slice = slice(
                self.__first_param_of_comp[i_c],
                self.__first_param_of_comp[i_c] + c.n_param,
            )
            res += [g[..., np.newaxis] for g in c.diff(nu, *params[param_slice])]
        return res
    ...
    def diff_diff(self, nu, *params):
        if not params:
            return None
        res = [
            [np.zeros((1, 1)) for i in range(self.n_param)] for i in range(self.n_param)
        ]
        for i_c, c in enumerate(self):
            param_slice = slice(
                self.__first_param_of_comp[i_c],
            ...
            for i in range(i_start, param_slice.stop):
                for j in range(param_slice.start, param_slice.stop):
                    res[i][j] = comp_diff_diff[i - i_start][j - i_start].reshape(-1, 1)
        return res
