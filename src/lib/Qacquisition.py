...
class QubicIntegrated(QubicPolyAcquisition):
    ...
    def _get_average_instrument_acq(self, nu):
        ...
        q = instr.QubicInstrument(d1, FRBW=0.25)
...
class QubicMultiAcquisitions:
    ...
    def __init__(self, dictionary, nsub, nrec=1, comps=[], H=None, sampling=None):
        ...
        _, allnus_edges_150, _, _, _, _ = qubic.compute_freq(
            150,
            Nfreq=int(self.nsub / 2) - 1,
            relative_bandwidth=self.dict["filter_relative_bandwidth"],
        )
        _, allnus_edges_220, _, _, _, _ = qubic.compute_freq(
            220,
            Nfreq=int(self.nsub / 2) - 1,
            relative_bandwidth=self.dict["filter_relative_bandwidth"],
        )
        ...
        _, _, nus150, _, _, _ = qubic.compute_freq(
            150,
            Nfreq=int(self.nrec / 2),
            relative_bandwidth=self.dict["filter_relative_bandwidth"],
        )
        _, _, nus220, _, _, _ = qubic.compute_freq(
            220,
            Nfreq=int(self.nrec / 2),
            relative_bandwidth=self.dict["filter_relative_bandwidth"],
        )
        ...
        ### Multi-frequency instrument
        self.multiinstrument = instr.QubicMultibandInstrumentTrapezoidalIntegration(
            self.dict
        )
        if sampling == None:
            self.sampling = qubic.get_pointing(self.dict)
        else:
            self.sampling = sampling
        self.scene = qubic.QubicScene(self.dict)
