...
def give_cl_cmb(r=0, Alens=1.0):
    ...
    if r:
        power_spectrum += (
            r
            * hp.read_cl(path + "Cls_Planck2018_unlensed_scalar_and_tensor_r1.fits")[
                :, :4000
            ]
        )
...
def fit_beta(tod, nsamples, obj, H_qubic, outputs):
    ...
    r = minimize(
        ...
        tol=1e-15,
        x0=np.array([1.0]),
        args=(obj, H_qubic, tod_norm, outputs, nsamples),
    )
...
class QubicPlanckMultiBandAcquisition:
    ...
    def _planck_correction(self, indice1, indice2):
        ...
        R_planck = ReshapeOperator(
            (12 * self.qubic.scene.nside**2, 3), (12 * self.qubic.scene.nside**2 * 3)
        )
        if indice1 == indice2:
            return R_planck
        else:
    ...
    def get_invntt_operator(
        ...
        if self.type == "Wide":
            ...
            invn_q = self.qubic.get_invntt_operator(
                det_noise, photon_noise150, photon_noise220
            )
            R = ReshapeOperator(invn_q.shapeout, invn_q.shape[0])
            invn_q = [R(invn_q(R.T))]

            invntt_planck143 = weight_planck * self.planck[0].get_invntt_operator(
                beam_correction=beam_correction[0], mask=mask, seenpix=seenpix
            )
            invntt_planck217 = weight_planck * self.planck[1].get_invntt_operator(
                beam_correction=beam_correction[0], mask=mask, seenpix=seenpix
            )
            R_planck = ReshapeOperator(
            ...
            for i in range(self.Nrec):
                invntt_planck = weight_planck * self.planck.get_invntt_operator(
                    beam_correction=beam_correction[i], mask=mask, seenpix=seenpix
                )
                R_planck = ReshapeOperator(
                    invntt_planck.shapeout, invntt_planck.shape[0]
                )
...
class QubicIntegrated(QubicPolyAcquisition):
    ...
    def __init__(self, d, Nsub=1, Nrec=1):
        ...
        if self.integration == "Trapeze":
            _, _, self.nueff, _, _, _ = qubic.compute_freq(
                self.d["filter_nu"],
                Nfreq=self.Nrec,
            ...
            _, self.nus_edge, self.nueff, _, _, _ = qubic.compute_freq(
...
class QubicFullBand(QubicPolyAcquisition):
    ...
    def __init__(self, d, Nsub, Nrec, relative_bandwidth=0.6138613861386139):
        ...
        if Nrec % 2 != 0 and Nrec != 1:
            raise TypeError(
                "You should put a number of reconstructed sub-bands divisible by 2"
            )
        ...
        if Nsub != 1:
            _, allnus150, _, _, _, _ = qubic.compute_freq(
                150, Nfreq=int(self.Nsub) - 1, relative_bandwidth=0.25
            )
            _, allnus220, _, _, _, _ = qubic.compute_freq(
                220, Nfreq=int(self.Nsub) - 1, relative_bandwidth=0.25
            )
        else:
            _, _, allnus150, _, _, _ = qubic.compute_freq(
                150, Nfreq=int(self.Nsub), relative_bandwidth=0.25
            )
            _, _, allnus220, _, _, _ = qubic.compute_freq(
                220, Nfreq=int(self.Nsub), relative_bandwidth=0.25
            )
    ...
    def get_invntt_operator(self, det_noise, photon_noise150, photon_noise220):
        ...
        invn150 = self.subacqs150[0].get_invntt_operator(
            det_noise=False, photon_noise=photon_noise150
        )
        invn220 = self.subacqs220[0].get_invntt_operator(
            det_noise=det_noise, photon_noise=photon_noise220
        )

        return (
            invn150 + invn220
        ) / 2  # factor 2 because it added twice the detector noise
