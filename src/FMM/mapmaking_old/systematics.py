...
def give_cl_cmb(r=0, Alens=1.0):
    power_spectrum = hp.read_cl(path_to_data + "Cls_Planck2018_lensed_scalar.fits")[
        :, :4000
    ]
    if Alens != 1.0:
    ...
    if r:
        power_spectrum += (
            r
            * hp.read_cl(
                path_to_data + "Cls_Planck2018_unlensed_scalar_and_tensor_r1.fits"
            )[:, :4000]
        )
    ...
    D = BlockDiagonalOperator(
        [
            DenseBlockDiagonalOperator(
                A, broadcast="rightward", shapein=(12 * nside**2, nc)
            ),
            DenseBlockDiagonalOperator(
                A, broadcast="rightward", shapein=(12 * nside**2, nc)
            ),
...
def get_mixing_operator(beta, nus, comp, nside, Amm=None, active=False):
    ...
    if nside_fit == 0:  # Constant indice on the sky
        ...
        D = DenseOperator(
            A[0],
            broadcast="rightward",
            shapein=(nc, 12 * nside**2, 3),
            shapeout=(1, 12 * nside**2, 3),
        )
...
class QubicFullBandSystematic(QubicPolyAcquisition):
    ...
    def __init__(self, d, Nsub, Nrec=1, comp=[], kind="DB", nu_co=None, H=None):
        ...
        for i in range(len(self.multiinstrument)):
            self.allfwhm[i] = (
                self.multiinstrument[i].get_convolution_peak_operator().fwhm
            )
    ...
    def get_components_operator(self, beta, nu, active=False):
        ...
        return r * get_mixing_operator(
            beta, nu, self.comp, self.scene.nside, Amm=None, active=active
        )
    ...
    def get_operator(self, beta, convolution, myfwhm=None, nu_co=None, comm=None):
        ...
        for ii, i in enumerate(self.nus):
            ...
            for j in range(self.nintegr):
                ...
                D = get_mixing_operator(
                    beta,
                    np.array([self.allnus[k]]),
                    comp=self.comp,
                    nside=self.nside,
                    active=False,
                )
...
class JointAcquisitionFrequencyMapMaking:
    ...
    def __init__(self, d, kind, Nrec, Nsub, H=None):
        ...
        self.qubic = QubicFullBandSystematic(
            self.d, comp=[], Nsub=self.Nsub, Nrec=self.Nrec, kind=self.kind, H=H
        )
...
class JointAcquisitionComponentsMapMaking:
    ...
    def __init__(self, d, kind, comp, Nsub, nus_external, nintegr, nu_co=None):
        ...
        # self.qubic = qubic
        self.qubic = QubicFullBandSystematic(
            self.d, comp=self.comp, Nsub=self.Nsub, Nrec=1, kind=self.kind, nu_co=nu_co
        )
        self.scene = self.qubic.scene
    ...
    def get_operator(self, beta, gain=None, fwhm=None, nu_co=None):
        ...
        He = self.external.get_operator(
            beta=beta, convolution=False, comm=mpidist, nu_co=nu_co
        )
