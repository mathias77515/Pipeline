...
class PipelineFrequencyMapMaking:
    ...
    def __init__(self, comm, file, params):
        ...
        self.joint_tod = JointAcquisitionFrequencyMapMaking(self.dict_in, self.params['QUBIC']['instrument'], self.params['QUBIC']['nsub_in'], self.params['QUBIC']['nsub_in'], H=None)
        ...
        self.joint = JointAcquisitionFrequencyMapMaking(self.dict_out, self.params['QUBIC']['instrument'], self.params['QUBIC']['nrec'], self.params['QUBIC']['nsub_out'], H=H)
        ...
        if self.params['QUBIC']['instrument'] == 'DB':
            qubic_noise = QubicDualBandNoise(self.dict_out, self.params['QUBIC']['npointings'], self.params['QUBIC']['NOISE']['detector_nep'])
        elif self.params['QUBIC']['instrument'] == 'UWB':
            qubic_noise = QubicWideBandNoise(self.dict_out, self.params['QUBIC']['npointings'], self.params['QUBIC']['NOISE']['detector_nep'])
    ...
    def _get_H(self):
        ...
        self.H_out = self.joint.get_operator(fwhm=self.fwhm_out, seenpix=self.seenpix)#, external_data=self.params['PLANCK']['external_data'])  
    ...
    def _get_preconditionner(self):
        ...
        if self.params['PCG']['preconditioner']:
            ...
            conditionner = np.zeros((self.params['QUBIC']['nrec'], 12*self.params['SKY']['nside']**2, 3))       
            ...
            for i in range(self.params['QUBIC']['nsub_out']):
                for j in range(self.params['QUBIC']['nsub_out']):
                    approx_hth[i] = self.joint.qubic.H[i].T * self.joint.qubic.invn220 * self.joint.qubic.H[j](vec)
            
            for irec in range(self.params['QUBIC']['nrec']):
                imin = irec * self.fsub 
                imax = (irec+1) * self.fsub 
                for istk in range(3):
                    conditionner[irec, self.seenpix, istk] = 1/(np.sum(approx_hth[imin:imax, self.seenpix, 0], axis=0))
    ...
    def run(self):
        ...
        if self.rank == 0:
            ...
            if len(self.externaldata.external_nus) != 0:
                ...
                self.nus_rec = np.array(list(self.nus_Q) + list(self.externaldata.external_nus))
                self.fwhm_rec = np.array(list(self.fwhm_rec) + list(fwhm_ext))


            self.plots.plot_frequency_maps(self.m_nu_in[:len(self.nus_Q)], self.s_hat[:len(self.nus_Q)], self.center, reso=15, nsig=3, 
                                           filename=self.plot_folder + f'/all_maps.png', figsize=(10, 5))
            ...
            dict_solution = {'maps':self.s_hat, 'maps_noise':self.s_hat_noise, 'nus':self.nus_rec, 'coverage':self.coverage, 
