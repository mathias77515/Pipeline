...
class PresetSims:
    ...
    def __init__(self, comm, seed, seed_noise, verbose=True):
        ...
        if self.rank == 0:
            if self.params["save_iter"] != 0:
                ...
                self.params["foldername"] = (
                    f"{self.params['Foregrounds']['Dust']['type']}_{self.params['Foregrounds']['Dust']['model_d']}_{self.params['QUBIC']['instrument']}_"
                    + self.params["foldername"]
                )
                create_folder_if_not_exists(self.params["foldername"])
            if (
                self.params["Plots"]["maps"] == True
                or self.params["Plots"]["conv_beta"] == True
            ):
        ...
        self.comps_in, self.comps_name_in = self._get_components_fgb(
            key="in", method=self.params["Foregrounds"]["Dust"]["type"]
        )
        self.comps_out, self.comps_name_out = self._get_components_fgb(
            key="out", method=self.params["Foregrounds"]["Dust"]["type"]
        )
        ...
        self.seenpix = (
            self.coverage / self.coverage.max() > self.params["PLANCK"]["thr_planck"]
        )
        ...
        if self.params["Foregrounds"]["Dust"]["nside_beta_out"] != 0:
            self.seenpix_beta = hp.ud_grade(
                self.seenpix, self.params["Foregrounds"]["Dust"]["nside_beta_out"]
            )

        ### Compute true components
        if self.verbose:
            self._print_message("    => Creating components")
        self.components_in, self.components_conv_in, _ = self._get_components(
            self.skyconfig_in
        )
        self.components_out, self.components_conv_out, self.components_iter = (
            self._get_components(self.skyconfig_out)
        )
        ...
        if self.params["Foregrounds"]["Dust"]["nside_beta_out"] != 0:
            self.coverage_beta = (
                self.get_coverage()
            )  # hp.ud_grade(self.seenpix_qubic, self.params['Foregrounds']['Dust']['nside_beta_out'])#self.get_coverage()
        else:
            self.coverage_beta = None

        C = HealpixConvolutionGaussianOperator(
            fwhm=self.params["PLANCK"]["fwhm_weight_planck"],
        ...
        centralpix = hp.ang2pix(
            self.params["SKY"]["nside"], self.center[0], self.center[1], lonlat=True
        )
        self.angmax = np.max(
            qss.get_angles(centralpix, pixsnum_seenpix, self.params["SKY"]["nside"])
        )
    ...
    def _get_preconditioner(self):
        ...
        if self.params["QUBIC"]["preconditionner"]:
            if self.params["Foregrounds"]["Dust"]["nside_beta_out"] == 0:
                conditionner = np.ones(
                    (len(self.comps_out), 12 * self.params["SKY"]["nside"] ** 2, 3)
                )
            else:
                conditionner = np.ones(
                    (3, 12 * self.params["SKY"]["nside"] ** 2, len(self.comps_out))
                )

                for i in range(conditionner.shape[0]):
                    for j in range(conditionner.shape[2]):
                        conditionner[i, self.seenpix_qubic, j] = self.coverage[
                            self.seenpix_qubic
                        ]  # /self.coverage[self.seenpix_qubic].max()
    ...
    def _angular_distance(self, pix):
        ...
        theta1, phi1 = hp.pix2ang(
            self.params["Foregrounds"]["Dust"]["nside_beta_out"], pix
        )
        pixmax = hp.vec2pix(uvcenter, lonlat=True)
        thetamax, phimax = hp.pix2ang(
            self.params["Foregrounds"]["Dust"]["nside_beta_out"], pixmax
        )
    ...
    def get_coverage(self):
        ...
        center = qubic.equ2gal(
            self.params["SKY"]["RA_center"], self.params["SKY"]["DEC_center"]
        )
        uvcenter = np.array(hp.ang2vec(center[0], center[1], lonlat=True))
        uvpix = np.array(
            hp.pix2vec(
                self.params["Foregrounds"]["Dust"]["nside_beta_out"],
                np.arange(
                    12 * self.params["Foregrounds"]["Dust"]["nside_beta_out"] ** 2
                ),
            )
        )
    ...
    def _get_noise(self):
        ...
        if self.params["QUBIC"]["instrument"] == "UWB":
            noise = QubicWideBandNoise(
                ...
                duration=np.mean(
                    [
                        self.params["QUBIC"]["NOISE"]["duration_150"],
                        self.params["QUBIC"]["NOISE"]["duration_220"],
                    ]
                ),
            )
        else:
    ...
    def _get_U(self):
        if self.params["Foregrounds"]["Dust"]["nside_beta_out"] == 0:
            U = (
                ReshapeOperator(
                    (len(self.comps_name) * sum(self.seenpix_qubic) * 3),
                    (len(self.comps_name), sum(self.seenpix_qubic), 3),
                )
                * PackOperator(
                    np.broadcast_to(
                        self.seenpix_qubic[None, :, None],
                        (len(self.comps_name), self.seenpix_qubic.size, 3),
                    ).copy()
                )
            ).T
        else:
            U = (
                ReshapeOperator(
                    (3 * len(self.comps_name) * sum(self.seenpix_qubic)),
                    (3, sum(self.seenpix_qubic), len(self.comps_name)),
                )
                * PackOperator(
                    np.broadcast_to(
                        self.seenpix_qubic[None, :, None],
                        (3, self.seenpix_qubic.size, len(self.comps_name)),
                    ).copy()
                )
            ).T
        return U
    ...
    def _get_tod(self):
        ...
        self.H = self.joint_in.get_operator(
            beta=self.beta_in, Amm=self.Amm_in, gain=self.g, fwhm=self.fwhm
        )
        ...
        ne = (
            self.joint_in.external.get_noise(seed=seed_pl)
        ...
        if (
            ...
            or self.params["QUBIC"]["convolution_out"]
        ):
            _r = ReshapeOperator(
                self.TOD_E.shape,
                (len(self.external_nus), 12 * self.params["SKY"]["nside"] ** 2, 3),
            )
            maps_e = _r(self.TOD_E)
            C = HealpixConvolutionGaussianOperator(
                fwhm=self.joint_in.qubic.allfwhm[-1],
                lmax=3 * self.params["SKY"]["nside"],
            )
    ...
    def _get_beta_input(self):
        ...
        self.nus_eff_in = np.array(
            list(self.joint_in.qubic.allnus) + list(self.joint_in.external.allnus)
        )
        self.nus_eff_out = np.array(
            list(self.joint_out.qubic.allnus) + list(self.joint_out.external.allnus)
        )

        if self.params["Foregrounds"]["Dust"]["model_d"] in ["d0", "d6"]:
            self.Amm_in = self._get_Amm(
                self.comps_in, self.comps_name_in, self.nus_eff_in, init=False
            )
            self.Amm_in[len(self.joint_in.qubic.allnus) :] = self._get_Amm(
                self.comps_in, self.comps_name_in, self.nus_eff_in, init=True
            )[len(self.joint_in.qubic.allnus) :]
            ...
            self.beta_in = np.zeros(
                (
                    12 * self.params["Foregrounds"]["Dust"]["nside_beta_in"] ** 2,
                    len(self.comps_in) - 1,
                )
            )
            for iname, name in enumerate(self.comps_name_in):
                if name == "CMB":
                    ...
                    self.beta_in[:, iname - 1] = self._spectral_index_mbb(
                        self.params["Foregrounds"]["Dust"]["nside_beta_in"]
                    )
                elif name == "Synchrotron":
                    self.beta_in[:, iname - 1] = self._spectral_index_pl(
                        self.params["Foregrounds"]["Dust"]["nside_beta_in"]
                    )
        else:
            raise TypeError(
                f"{self.params['Foregrounds']['Dust']['model_d']} is not yet implemented..."
            )
    ...
    def _get_components(self, skyconfig):
        ...
        components = np.zeros(
            (len(skyconfig), 12 * self.params["SKY"]["nside"] ** 2, 3)
        )
        components_conv = np.zeros(
            (len(skyconfig), 12 * self.params["SKY"]["nside"] ** 2, 3)
        )
        ...
        if (
            self.params["QUBIC"]["convolution_in"]
            or self.params["QUBIC"]["convolution_out"]
        ):  # or self.params['QUBIC']['fake_convolution']:
            C = HealpixConvolutionGaussianOperator(
                fwhm=self.joint_in.qubic.allfwhm[-1],
                lmax=3 * self.params["SKY"]["nside"],
            )
        ...
        mycls = give_cl_cmb(
            r=self.params["CMB"]["r"], Alens=self.params["CMB"]["Alens"]
        )
        ...
        for k, kconf in enumerate(skyconfig.keys()):
            if kconf == "cmb":
                ...
                cmb = hp.synfast(
                    mycls, self.params["SKY"]["nside"], verbose=False, new=True
                ).T
                ...
                map_Dust = (
                    np.array(
                        sky.get_emission(
                            self.params["Foregrounds"]["Dust"]["nu0_d"] * u.GHz, None
                        ).T
                        * utils.bandpass_unit_conversion(
                            self.params["Foregrounds"]["Dust"]["nu0_d"] * u.GHz,
                            None,
                            u.uK_CMB,
                        )
                    )
                    * self.params["Foregrounds"]["Dust"]["amplification_d"]
                )
                ...
                sky = pysm3.Sky(
                    nside=self.params["SKY"]["nside"],
                    preset_strings=[
                        self.params["Foregrounds"]["Synchrotron"]["model_s"]
                    ],
                    output_unit="uK_CMB",
                )
                ...
                map_sync = (
                    np.array(
                        sky.get_emission(
                            self.params["Foregrounds"]["Synchrotron"]["nu0_s"] * u.GHz,
                            None,
                        ).T
                        * utils.bandpass_unit_conversion(
                            self.params["Foregrounds"]["Synchrotron"]["nu0_s"] * u.GHz,
                    ...
                    * self.params["Foregrounds"]["Synchrotron"]["amplification_s"]
                )
                ...
                m = hp.ud_grade(
                    hp.read_map("data/CO_line.fits") * 10, self.params["SKY"]["nside"]
                )
                mP = polarized_I(
                    m,
                    self.params["SKY"]["nside"],
                    polarization_fraction=self.params["Foregrounds"]["CO"][
    ...
    def _get_components_fgb(self, key, method="blind"):
        ...
        if self.params["Foregrounds"]["Dust"][f"Dust_{key}"]:
            comps += [
                c.Dust(
                    nu0=self.params["Foregrounds"]["Dust"]["nu0_d"],
                    temp=20,
                    beta_d=beta_d,
                )
            ]
        ...
        if self.params["Foregrounds"]["Synchrotron"][f"Synchrotron_{key}"]:
            comps += [
                c.Synchrotron(nu0=self.params["Foregrounds"]["Synchrotron"]["nu0_s"])
            ]
            comps_name += ["Synchrotron"]

        if self.params["Foregrounds"]["CO"][f"CO_{key}"]:
            comps += [
                c.COLine(nu=self.params["Foregrounds"]["CO"]["nu0_co"], active=False)
            ]
            comps_name += ["CO"]
    ...
    def _get_convolution(self):
        ...
        if self.params["QUBIC"]["convolution_out"]:
            self.fwhm_recon = np.sqrt(
                self.joint_in.qubic.allfwhm**2
                - np.min(self.joint_in.qubic.allfwhm) ** 2
            )
    ...
    def _get_input_gain(self):
        ...
        if self.params["QUBIC"]["instrument"] == "wide":
            self.g = np.random.normal(
                1, self.params["QUBIC"]["GAIN"]["sig_gain"], self.joint_in.qubic.ndets
            )
            ...
            self.g = np.random.normal(
                1,
                self.params["QUBIC"]["GAIN"]["sig_gain"],
                (self.joint_in.qubic.ndets, 2),
            )
        ...
        if self.params["QUBIC"]["GAIN"]["fit_gain"]:
            g_err = 0.2
            self.g_iter = np.random.uniform(
                self.g - g_err / 2, self.g + g_err / 2, self.g.shape
            )
    ...
    def _get_x0(self):
        ...
        if self.params["Foregrounds"]["Dust"]["model_d"] in ["d0", "d6"]:
            ...
            if self.params["Foregrounds"]["Dust"]["Dust_out"]:
                self.beta_iter = np.append(
                    self.beta_iter,
                    np.random.normal(
                        self.params["Foregrounds"]["Dust"]["beta_d_init"][0],
                        self.params["Foregrounds"]["Dust"]["beta_d_init"][1],
                        1,
                    ),
                )
            if self.params["Foregrounds"]["Synchrotron"]["Synchrotron_out"]:
                self.beta_iter = np.append(
                    self.beta_iter,
                    np.random.normal(
                        self.params["Foregrounds"]["Synchrotron"]["beta_s_init"][0],
                        self.params["Foregrounds"]["Synchrotron"]["beta_s_init"][1],
                        1,
                    ),
                )
        else:
            ...
            for iname, name in enumerate(self.comps_name_out):
                if name == "CMB":
                    ...
                    self.beta_iter[:, iname - 1] = self._spectral_index_mbb(
                        self.params["Foregrounds"]["Dust"]["nside_beta_out"]
                    )
                elif name == "Synchrotron":
                    self.beta_iter[:, iname - 1] = self._spectral_index_pl(
                        self.params["Foregrounds"]["Dust"]["nside_beta_out"]
                    )
        ...
        if self.params["Foregrounds"]["Dust"]["nside_beta_out"] == 0:
            ...
            C1 = HealpixConvolutionGaussianOperator(
                fwhm=self.fwhm_rec, lmax=3 * self.params["SKY"]["nside"]
            )
            C2 = HealpixConvolutionGaussianOperator(
                fwhm=self.params["INITIAL"]["fwhm0"],
                lmax=3 * self.params["SKY"]["nside"],
            )
            ...
            for i in range(len(self.comps_out)):
                if self.comps_name_out[i] == "CMB":
                    ...
                    self.components_iter[i, self.seenpix_qubic, 1:] *= self.params[
                        "INITIAL"
                    ]["qubic_patch_cmb"]
                    self.components_iter[i, self.seenpix_qubic, 1:] += np.random.normal(
                        0,
                        self.params["INITIAL"]["sig_map_noise"],
                        self.components_iter[i, self.seenpix_qubic, 1:].shape,
                    )
            ...
            for i in range(len(self.comps_out)):
                if self.comps_name_out[i] == "CMB":
                    ...
                    self.components_iter[:, :, i] = C2(
                        C1(self.components_iter[:, :, i].T)
                    ).T
                    self.components_iter[1:, self.seenpix, i] *= self.params["INITIAL"][
                        "qubic_patch_cmb"
                    ]

                elif self.comps_name_out[i] == "Dust":
                    self.components_iter[:, :, i] = (
                        C2(C1(self.components_iter[:, :, i])).T
                        + np.random.normal(
                            0,
                            self.params["INITIAL"]["sig_map_noise"],
                            self.components_iter[:, :, i].T.shape,
                        ).T
                    )
                    self.components_iter[1:, self.seenpix, i] *= self.params["INITIAL"][
                        "qubic_patch_Dust"
