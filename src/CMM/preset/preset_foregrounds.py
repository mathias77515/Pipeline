...
class PresetFG:
    ...
    def __init__(self, preset_tools, preset_qubic, seed):
        ...
        self.components_model_in, self.components_name_in = (
            self.preset_qubic._get_components_fgb(key="in")
        )
        self.components_model_out, self.components_name_out = (
            self.preset_qubic._get_components_fgb(key="out")
        )

        ### Compute true components
        self.preset_tools._print_message("    => Creating components")
        self.components_in, self.components_convolved_in, _ = self._get_components(
            self.skyconfig_in
        )
