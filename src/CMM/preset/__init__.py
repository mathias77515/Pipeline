    ...
    def initialize(self):
        ...
        if self.tools.rank == 0:
            if self.tools.params['save_iter'] != 0:
                ...
                self.tools.create_folder_if_not_exists('src/CMM/' + self.tools.params['foldername'] + '/maps/')
            if self.tools.params['Plots']['maps'] == True or self.tools.params['Plots']['conv_beta'] == True:
                self.tools.create_folder_if_not_exists(f'src/CMM/jobs/{self.job_id}/I')
                self.tools.create_folder_if_not_exists(f'src/CMM/jobs/{self.job_id}/Q')
                self.tools.create_folder_if_not_exists(f'src/CMM/jobs/{self.job_id}/U')
                self.tools.create_folder_if_not_exists(f'src/CMM/jobs/{self.job_id}/allcomps')
                self.tools.create_folder_if_not_exists(f'src/CMM/jobs/{self.job_id}/A_iter')
