...
class Pipeline:
    ...
    def _callback(self, x):
        ...
        if self.preset.tools.rank == 0:
            if (self.nfev%1) == 0:
                print(f"Iter = {self.nfev:4d}   x = {[np.round(x[i], 5) for i in range(len(x))]}   qubic log(L) = {np.log(np.round(self.chi2.Lqubic, 5))}  planck log(L) = {np.log(np.round(self.chi2.Lplanck, 5))}")
            self.nfev += 1
