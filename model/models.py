...
class Sky(ForeGroundModels):
    ...
    def model(self, fnu1d, fnu2d, fnu1s, fnu2s, fsky):
        
        #print(int(self.params['Sky']['CMB']['cmb']))
        Dl_cmb = self.get_Dl_cmb() * int(self.params['Sky']['CMB']['cmb'])
        Dl_fg = self.get_Dl_fg(self.ell, fnu1d, fnu2d, fnu1s, fnu2s)#, fsky)
    ...
    def get_Dl(self, fsky=0.01):
        ...
        for inu, nu in enumerate(self.nus):
            for jnu, nu in enumerate(self.nus):
                ...
                Dl[k] = self.model(fd[inu], fd[jnu], fs[inu], fs[jnu], fsky)
