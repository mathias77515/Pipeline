...
class DataFMM:
    ...
    def auto_spectra_noise_reduction(self, mean_data, mean_noise):
        ...
        for i in range(self.param_sampling['simu']['nrec']):
...
class DataCMM:
    ...
    def get_power_spectra(self):
        ...
        if self.param_sampling['simu']['noise']:
            mean_ps_noise, error_ps_noise, _ = self.compute_data(self.path_noise)
            mean_ps_sky = self.auto_spectra_noise_reduction(mean_ps_sky, mean_noise)
...
class Spectra:
    ...
    def __init__(self):
        ...
        with open('sampling_config.yml', "r") as stream:
            self.param_sampling = yaml.safe_load(stream)
        self.pipeline = self.param_samplings['data']['pipeline']
        ...
        if self.pipeline == 'FMM':
            ...
            self.mean_data, self.error_data, self.map_data = DataFMM().compute_data(self.param_sampling['data']['path_sky'])
            self.mean_noise, self.error_noise, _ = DataFMM().compute_data(self.param_sampling['data']['path_noise'])
        self.ell, self.namaster = NamasterEll().ell()


    def compute_spectrum(self, map1, map2):
        '''
        Function to compute the cross-spectrum or the auto-spectrum if map2=None
        '''

        _, Dl, _ = self.Namaster.get_spectra(map=map1, map2=map2, 
                                                 beam_correction=None, 
                                                 pixwin_correction=self.param_samplings['Spectrum']['pixwin_correction'],
                                                 verbose=False)
    ...
    def cross_sectra_convo(self, mean_sky, map_data):
        ...
        my_dict, _ = self.get_dict()
        joint = acq.JointAcquisitionFrequencyMapMaking(my_dict, self.data_parameters['QUBIC']['type'], self.data['parameters']['QUBIC']['nrec'], self.data['parameters']['QUBIC']['nsub'])
        allfwhm = joint.qubic.allfwhm
                ...
                if i != j:
                    for real in range(self.param_sampling['data']['n_real']):
                        ...
                        if allfwhm[i*self.fsub]<allfwhm[j*self.fsub] :
                            C = HealpixConvolutionGaussianOperator(fwhm=allfwhm[j*self.fsub])
                            convoluted_map = C*map_data[real][i]
                            cross_spect.append(self.namaster.get_spectra(map=convoluted_map.T, map2=map_data[real][j].T)[1][:, 2])
                        else:
                            C = HealpixConvolutionGaussianOperator(fwhm=allfwhm[i*self.fsub])
                            convoluted_map = C*map_data[real][j]
                            cross_spect.append(self.namaster.get_spectra(map=map_data[real][i].T, map2=convoluted_map.T)[1][:, 2])
                    mean_sky[i, j, :] = np.mean(cross_spect, axis=0)
...
class MCMC:
    ...
    def __init__(self):
        ...
        self.mean_ps_sky, self.error_ps_noise, self.error_ps_sky = DataFMM().get_power_spectra()
