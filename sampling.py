...
class Data:
    ...
    def __init__(self):
        ...
        self.path_data = self.param['data']['path_data']
        self.path_noise = self.param['data']['path_noise']
    ...
    def find_data(self):
        ...
        data_names = os.listdir(self.path_data)

        data = pickle.load(open(self.path_data + '/' + data_names[0], 'rb'))
    ...
    def get_data(self):
        ...
        mean_data, error_data, map_data = self.compute_data(self.path_data)

        if self.data['parameters']['QUBIC']['convolution']:
            mean_data = self.cross_sectra_convo(mean_data, map_data)

        if self.param['simu']['noise']:
            mean_noise, error_noise, _ = self.compute_data(self.path_noise)
...
class MCMC:
    ...
    def __init__(self):
        ...
        self.ndim, self.sky_parameters_names, self.sky_parameters_all_names = self.ndim_and_parameters_names()
    ...
    def ndim_and_parameters_names(self):
        ...
        return ndim, sky_parameters_names, sky_parameters_all_names
    ...
    def __call__(self):
        ...
        with Pool() as pool:
            sampler = emcee.EnsembleSampler(nwalkers, self.ndim, log_prob_fn = self.chi2, pool = pool, moves = [(emcee.moves.StretchMove(), self.param['MCMC']['stretch_move_factor']), (emcee.moves.DESnookerMove(gammas=self.param['MCMC']['snooker_move_gamma']), 1 - self.param['MCMC']['stretch_move_factor'])])
            sampler.run_mcmc(p0, mcmc_steps, progress=True)

        samples_flat = sampler.get_chain(flat = True, discard = self.param['MCMC']['discard'], thin = self.param['MCMC']['thin'])
        ...
        for j in range(self.ndim):
            for i in range(nwalkers):
                ...
                ax[j].set_title(self.sky_parameters_names[j])
        ...
        # Triangle plot
        plt.figure()
        s = MCSamples(samples=samples_flat, names=self.sky_parameters_names, labels=self.sky_parameters_names)
...
class NestedSampling:
    ...
    def chi2(self, tab):
        ...
        # Return the chi2 function
        if self.param['simu']['name'] == 'CMB':
            return - 0.5 * np.sum(((self.mean_data - CMB(self.ell).model_cmb(r, Alens))/(self.error_noise))**2)
        if self.param['simu']['name'] == 'Dust':
            return - 0.5 * np.sum(((self.mean_data - Dust(self.ell).model_dust(Ad, alphad, betad, deltad, nu0_d))/(self.error_noise))**2)
        if self.param['simu']['name'] == 'Sky':
            return - 0.5 * np.sum(((self.mean_data - (CMB(self.ell).model_cmb(r, Alens) + Dust(self.ell).model_dust(Ad, alphad, betad, deltad, nu0_d)))/(self.error_noise))**2)

    def ptform_uniform(self, u):
        ptform = []

        for iname in self.sky_parameters_all_names:
            if self.param['SKY_PARAMETERS'][iname][0] is True:
                ptform.append(u[0]*self.param['SKY_PARAMETERS'][iname][2] - self.param['SKY_PARAMETERS'][iname][1])
        return ptform
    ...
    def __call__(self):
        ...
        nlive = self.param['NS']['nlive']
        ell = self.ell
        print(Data().find_data()['parameters'])
        
        if self.param['NS']['DynamicNS'] is True:
            print('Dynamic Nested Sampling !!!')
            with Pool() as pool:
                sampler_ns = DynamicNestedSampler(self.chi2, self.ptform_uniform, self.ndim, pool = pool, nlive = nlive, queue_size=self.param['NS']['queue_size'], bound=self.param['NS']['bound'])
                sampler_ns.run_nested()
        else:
            with Pool() as pool:
                print('Nested Sampling !')
                sampler_ns = NestedSampler(self.chi2, self.ptform_uniform, self.ndim, pool = pool, nlive = nlive, queue_size=self.param['NS']['queue_size'], bound=self.param['NS']['bound'])
                sampler_ns.run_nested()

        results = sampler_ns.results

        # Plot the traceplots
        fig, axes = dyplot.traceplot(results, show_titles=True, labels = self.sky_parameters_names,
        ...
        if not os.path.isdir(path_plot):
            os.makedirs(path_plot)
        plt.savefig(self.param['data']['path'] + path_plot + f'/traceplot_Nreal={n_real}')

        # Triangle plot
        fig, axes = plt.subplots(3, 3)
        axes = axes.reshape((3, 3))
        fg, ax = dyplot.cornerplot(results, color='blue', title_fmt = '.4f', show_titles=True, labels = self.sky_parameters_names,
