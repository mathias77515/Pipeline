...
class BBPip:
    ...
    def __init__(self, path, path_noise):
        ...
        model = self.cmb_model(0, 1) + self.fg_model(13.4, -0.1, 1.54, 1, 353)
    ...
    def loglike(self, x):
        ...
        _r = (self.DlBB - ymodel) / np.std(self.Nl, axis=0)
        #c = self.knox_covariance(cmbtheo)
        L = logprob - np.sum(_r**2)
...
pip = BBPip(path=os.path.dirname(os.getcwd()) + '/E2E_nrec2/cmbdust/spectrum/',
...
if pip.rank == 0:
    ...
    with open(f'{filename}.pkl', 'wb') as handle:
        pickle.dump({'chains':chains, 'chains_flat':chains_flat, 'ell':pip.ell, 
                     ...
                     'nus':pip.nus}, handle, protocol=pickle.HIGHEST_PROTOCOL)
